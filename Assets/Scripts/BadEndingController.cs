...
// Bad Ending — hết giờ trước khi Player kịp uống cure + thoát ra. Đặc nhiệm xử lý để ngăn
// virus lan ra ngoài. Cùng phong cách "camera an ninh HELIX CORP" như IntroCinematicController.cs,
// nhưng tông màu đỏ/tối, dựng UI hoàn toàn bằng code lúc runtime.
public class BadEndingController : MonoBehaviour
{
    ...
    readonly Color crimson = new(0.95f, 0.15f, 0.18f, 1f);
    readonly Color paleRed = new(1f, 0.72f, 0.72f, 1f);
    readonly Color ash = new(0.55f, 0.55f, 0.58f, 1f);
    ...
    void BuildPresentation()
    {
        ...
        var atmosphere = CreatePanel("Atmosphere", root, new Color(0.06f, 0.015f, 0.015f, 1f), Vector2.zero, Vector2.one);
        ...
        CreatePanel("Top Accent", root, new Color(crimson.r, crimson.g, crimson.b, 0.4f), new Vector2(0f, 0.905f), new Vector2(1f, 0.908f));
        CreatePanel("Bottom Accent", root, new Color(crimson.r, crimson.g, crimson.b, 0.25f), new Vector2(0f, 0.092f), new Vector2(1f, 0.095f));

        scanlines = new RectTransform[18];
        for (int i = 0; i < scanlines.Length; i++)
        {
            float y = 0.1f + i * 0.047f;
            Image line = CreatePanel("Scanline " + (i + 1), root, new Color(1f, 0.3f, 0.3f, i % 4 == 0 ? 0.04f : 0.014f), new Vector2(0f, y), new Vector2(1f, y));
            line.rectTransform.sizeDelta = new Vector2(0f, i % 4 == 0 ? 2f : 1f);
            scanlines[i] = line.rectTransform;
        }

        CreateText("System Label", root, font, "HELIX SECURE NETWORK  //  TERMINATION LOG", 19, FontStyle.Bold,
            new Color(crimson.r, crimson.g, crimson.b, 0.65f), TextAnchor.MiddleLeft,
            new Vector2(0.035f, 0.93f), new Vector2(0.65f, 0.985f));

        CreateText("Status", root, font, "SIGNAL LOST", 17, FontStyle.Bold,
            new Color(crimson.r, crimson.g, crimson.b, 0.85f), TextAnchor.MiddleRight,
        ...
        navigationText = CreateText("Navigation", root, font, "ENTER  -  NEXT PANEL     ESC  -  SKIP", 17, FontStyle.Normal,
            new Color(0.8f, 0.72f, 0.72f, 0.7f), TextAnchor.MiddleRight,
            new Vector2(0.55f, 0.015f), new Vector2(0.965f, 0.075f));

        // Panel 0 — Status: quarantine window closed
        ...
        CreateText("Status Heading", statusGroup.transform as RectTransform, font, "CONTAINMENT WINDOW CLOSED", 26, FontStyle.Bold,
            crimson, TextAnchor.MiddleLeft, new Vector2(0.14f, 0.68f), new Vector2(0.86f, 0.78f));
        CreateText("Status Location", statusGroup.transform as RectTransform, font, "FACILITY-07  /  RESPONSE TEAM DEPLOYED", 40, FontStyle.Bold,
            paleRed, TextAnchor.MiddleLeft, new Vector2(0.14f, 0.54f), new Vector2(0.86f, 0.69f));
        CreatePanel("Status Rule", statusGroup.transform as RectTransform, new Color(crimson.r, crimson.g, crimson.b, 0.8f),
        ...
        statusBody = CreateText("Status Body", statusGroup.transform as RectTransform, font, string.Empty, 25, FontStyle.Normal,
            new Color(0.92f, 0.82f, 0.82f, 1f), TextAnchor.UpperLeft,
        ...
        statusFullText =
            ...
            "> NEXUS FLUID SIGNATURE STILL ACTIVE.\n" +
            "> SPECIAL RESPONSE TEAM DISPATCHED. NO EXCEPTIONS.";

        // Panel 1 — Epilogue
        epilogueGroup = CreateGroup("Epilogue", root);
        CreateBackdrop(epilogueGroup.transform as RectTransform, epilogueBackdrop, 0.45f);
        CreateText("Epilogue Heading", epilogueGroup.transform as RectTransform, font, "FINAL ENTRY  //  CONTAINMENT WING", 24, FontStyle.Bold,
            ash, TextAnchor.MiddleLeft, new Vector2(0.14f, 0.68f), new Vector2(0.86f, 0.78f));
        CreateText("Epilogue Name", epilogueGroup.transform as RectTransform, font, "DR. ALEX RIVERA", 56, FontStyle.Bold,
            paleRed, TextAnchor.MiddleLeft, new Vector2(0.14f, 0.5f), new Vector2(0.86f, 0.69f));
        CreatePanel("Epilogue Rule", epilogueGroup.transform as RectTransform, new Color(crimson.r, crimson.g, crimson.b, 0.75f),
            new Vector2(0.14f, 0.485f), new Vector2(0.86f, 0.489f));
        epilogueFullText =
            "Alex reached the exit half a second too late.\n\n" +
            "The response team's directive was absolute:\n" +
            "no infected signature leaves Facility-07 alive.";
        epilogueBody = CreateText("Epilogue Body", epilogueGroup.transform as RectTransform, font, string.Empty,
            26, FontStyle.Normal, new Color(0.92f, 0.82f, 0.82f, 1f), TextAnchor.UpperLeft,
        ...
        CreateText("Specimen", titleGroup.transform as RectTransform, font, "SUBJECT: DR. ALEX RIVERA  //  STATUS: TERMINATED", 19, FontStyle.Bold,
            new Color(crimson.r, crimson.g, crimson.b, 0.75f), TextAnchor.LowerCenter,
            new Vector2(0.1f, 0.67f), new Vector2(0.9f, 0.77f));
        Text title = CreateText("Title", titleGroup.transform as RectTransform, font, "CONTAINMENT COMPLETE", 66, FontStyle.Bold,
            paleRed, TextAnchor.MiddleCenter, new Vector2(0.08f, 0.34f), new Vector2(0.92f, 0.57f));
        var titleOutline = title.gameObject.AddComponent<Outline>();
        titleOutline.effectColor = new Color(crimson.r, crimson.g, crimson.b, 0.45f);
        titleOutline.effectDistance = new Vector2(3f, -3f);
        CreateText("Tagline", titleGroup.transform as RectTransform, font, "THE ECHO CLAIMS ANOTHER", 21, FontStyle.Normal,
            new Color(crimson.r, crimson.g, crimson.b, 0.85f), TextAnchor.UpperCenter,
            new Vector2(0.15f, 0.25f), new Vector2(0.85f, 0.38f));

        flashOverlay = CreatePanel("Glitch Flash", root, new Color(crimson.r, crimson.g, crimson.b, 0f), Vector2.zero, Vector2.one);
    ...
    IEnumerator SwitchPanel(int nextPanel)
    {
        ...
        flashOverlay.color = new Color(crimson.r, crimson.g, crimson.b, 0.1f);
        ...
        flashOverlay.color = new Color(crimson.r, crimson.g, crimson.b, 0f);
    ...
    void AnimateRecIndicators()
    {
        ...
        foreach (var dot in recIndicators)
        {
            ...
            dot.color = new Color(crimson.r, crimson.g, crimson.b, alpha);
        }
    }
    ...
    void GoToMainMenu()
    {
        ...
        if (Application.CanStreamedLevelBeLoaded(mainMenuScene))
            ...
            Debug.LogError($"BadEnding could not load scene '{mainMenuScene}'. Add it to Build Settings.");
    }
    ...
    {
        Image dot = CreatePanel("Rec Dot", parent, crimson, new Vector2(0.045f, 0.9f), new Vector2(0.062f, 0.928f));
        recIndicators.Add(dot);

        CreateText("Rec Label", parent, uiFont, "REC", 17, FontStyle.Bold, crimson, TextAnchor.MiddleLeft,
